...
<form action="<%=request.getContextPath()%>/toon">
    <div class="toon-week-box">
        ...
        <button class="toon-week" value="5" name="week">토</button>
        <button class="toon-week" value="6" name="week">일</button>
        <button class="toon-week" value="7" name="week">열흘</button>
    </div>
    <div class="toon-box">
    	<c:if test="${tlist.size()!=0}">
	        <ul class="toon-list">
	        	<c:forEach var="toon" items="${wlist}">
		            <li class="toon-item">
		                <a class="toon-link" href="<%=request.getContextPath()%>/toon/ep?title=${toon.t_title}&week=${pm.cri.week}&page=${pm.cri.page}&type=${pm.cri.type}&search=${pm.cri.search}">
